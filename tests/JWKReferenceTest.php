...
class JWTVerifierMock extends \Cerpus\JWTSupport\JWTVerifier {
    protected function downloadResource($url) {
        if ($url === 'https://example.com/jwks.json') {
            ...
            return parent::downloadResource($url);
        }
    }
}
...
class JWKReferenceTest extends \PHPUnit_Framework_TestCase {
    public function testReferenceJwk() {
        $jwtSupport = new JWTVerifierMock(["test" => "https://example.com/jwks.json"]);
        ...
        $jwtSupport->getVerifierByName("test")->setLeeway(PHP_INT_MAX/4);
        $validJwt = $jwtSupport->verify($exampleToken);
        $this->assertTrue($validJwt instanceof \Cerpus\JWTSupport\ValidJWT);
        $this->assertEquals("test", $validJwt->getName());
        $tokenData = $validJwt->getJwt();
        print_r($tokenData);
    }
}